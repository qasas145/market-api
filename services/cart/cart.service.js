...
async function addToCart(userId, newProducts) {

  for (let newProduct of newProducts) {
    const isValidSchema = await verifySchema(productSchema, newProduct);
    if (!isValidSchema) throw new HttpError(StatusCodes.BAD_REQUEST, `product ${newProduct} schema is not valid`);
